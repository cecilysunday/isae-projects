...
// Then, once the particles re-settle, push an object vertically through the 
// material at a constant velocity and record the force on the object.
//
// Additional notes:
...
int UpdateWallProperties(ChSystemMulticoreSMC* msystem, const ConfigParameters& cp) {
	// Get pointers to the cylinder base and roof and the penetometer assuming that base = 0, roof = 2, and pentrometer = 3
	std::shared_ptr<ChBody> base = msystem->Get_bodylist().at(1);
	std::shared_ptr<ChBody> roof = msystem->Get_bodylist().at(2);
	std::shared_ptr<ChBody> penetrometer = msystem->Get_bodylist().at(3);

	// Change the collision properties of the roof
	roof->SetCollide(false);
	roof->GetAssets().clear();

	// Identify the old and new height of the penetrometer based on the actual surface height of the beads
	ChVector<> pold = penetrometer->GetPos();
	ChVector<> pnew = ChVector<>(pold.x(), pold.y(), pold.z() + GetSurfaceHeight(msystem) - cp.clength_z);

	// Change the position, motion and collision properties of the penetrometer
	penetrometer->SetPos(pnew);
	penetrometer->SetBodyFixed(false);
	penetrometer->SetCollide(true);

	// Attach a motor between the penetrometer and the base of the container
	auto motor = chrono_types::make_shared<ChLinkMotorLinearSpeed>();
	motor->Initialize(penetrometer, base, ChFrame<>(ChVector<>(0, 0, 0), Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
	msystem->Add(motor);

	auto mspeed = chrono_types::make_shared<ChFunction_Const>(cp.ivel);
	motor->SetSpeedFunction(mspeed);
	motor->SetAvoidPositionDrift(0);

	// Add a bounding box to the system so particles don't fly too far away
	double hlim = pnew.z() + cp.ilength_z;
...
std::pair<size_t, size_t> CreateContainer(ChSystemMulticoreSMC* msystem, const ConfigParameters& cp) {
	...
	ChVector<> spntr = ChVector<>(cp.ilength_r, cp.ilength_r, cp.ilength_z / 2.0);
	...
	ChVector<> ppntr = ChVector<>(0, 0, cp.clength_z + cp.ilength_z / 2.0);
	...
	AddWall(--id, msystem, cp, spntr, ppntr, false, false);
...
int main(int argc, char* argv[]) {
	...
	#ifdef CHRONO_IRRLICHT
	ChIrrApp application(&msystem, L"PENETROMETER", irr::core::dimension2d<irr::u32>(800, 600), VerticalDir::Z);
	...
	// Update the collision and position properties of the penetrometer and container
	if (UpdateWallProperties(&msystem, cp) != 0) {
		fprintf(stderr, "\nERR: Error updating the container properties following the initial re-settle phase. \n");
		return -1;
	}

	// Export the penetrometer data at the end of the re-settle phase
	...
	double zi = msystem.Get_bodylist().at(wlist.second)->GetPos().z();

	// LOOP 2: Iterate through the simulation for a second time to push the penetrometer through the material
	while (time < sim_duration) {
		...
		// Save and write the penetrometer data every cp.time_loop step
		if (ArchiveSingleBody(msystem, cp.proj_path, wlist.second) != 0) {
			fprintf(stderr, "\nERR: Error archiving projectile run-time data \n");
			return -1;
		}
		...
		// Exit the loop if the penetrometer reaches a depth that is greater than half its height
		double zf = msystem.Get_bodylist().at(wlist.second)->GetPos().z();
		if (zi - zf > cp.ilength_z / 2.0) break;
	}
	...
	// Print the particle-particle contact information for all contacts at the end of the simulation
	if (PrintContactInfo(msystem, cp.proj_path, time, false) != 0) {
		fprintf(stderr, "Error writing contact info\n");
		return -1;
	}
