...
std::pair<size_t, size_t> FillContainer(ChSystemMulticoreSMC* msystem, const ConfigParameters& cp) {
	...
	std::normal_distribution<double> distribution_2(cp.grad*cp.gsr, cp.gdia_std_s*cp.grad*cp.gsr);
	...
	double mass_total=0.;
	double mass_s=0.0;
	double fc_target=cp.fc;
	std::cout<<"fc_target"<<fc_target<<"\n";
	std::cout<<"gsr"<<cp.gsr<<"\n";
	double fc=0.0;
	double mass=0.0;
	int nb_large=0;
						...
						if (pos_next.x() <= cp.clength_x / 2.0 - marg && pos_next.x() >= -cp.clength_x / 2.0 + marg) {
							...
							while (rad<=0.0){
								if (fc>fc_target){
									...
									rad=distribution_2(generator);
								}
							}
							AddSphere(id++, msystem, cp, rad, pos_next, ChRandomXYZ(cp.gvel));
							mass = msystem->Get_bodylist().at(msystem->Get_bodylist().size() - 1)->GetMass();
							mass_total+=mass;
							if (fc<fc_target){
								mass_s+=mass;
								nb_small+=1;
	...
	std::cout<<"FC final = "<<fc<<"\n";
	std::cout<<"Nb large = "<<nb_large<<", Nb small = "<<nb_small<<"\n";
